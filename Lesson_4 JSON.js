...
//
const fs = require('fs');
const data = fs.readFileSync('./task2.json'); //чтение файла
let jsondata = JSON.parse(data); //перевод данных в JSON

let set = new Set(jsondata.map(JSON.stringify));
let uniqArray = Array.from(set).map(JSON.parse);

uniqArray.forEach(user => {
